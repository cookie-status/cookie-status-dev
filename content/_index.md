...
Brave no longer blocks same-site resource loads even if the resources are in the filter lists. Safari can be configured to hide IP addresses from known trackers. Firefox trims referrer to eTLD+1 if the referrer has a tracking parameter in the URL.
